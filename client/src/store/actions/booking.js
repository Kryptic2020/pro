...
export const fetchDates = () => dispatch => {
	axios.get('/api/calendar/get').then((res) => {
		...
		let s = [];
		rawData.map((j) => {
			if (!j.isBooked) { if (s.includes(j.service) === false) s.push(j.service); }
		});
		dispatch(openServices(s));
		let x = [];
		rawData.map((k) => {
			if (x.includes(k.service) === false) x.push(k.service); 
		});
		dispatch(allServices(x));
		let n = [];
		rawData.map((profile) => {
			//n.push(new Date(profile.date));
			n.push(new Date(profile.date.split('/').reverse().join('/')), profile.service);
		}); //console.log(n);
