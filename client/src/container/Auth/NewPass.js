...
class NewPass extends Component {
  ...
  componentDidMount() {
  	 this.setState({ token: new URLSearchParams(this.props.location.search).get('key') });
  ...
  //Password  - Component 1
	inputChangeHandler = (event, input) => {
  	...
  	return (
  		<div>
  			<div >
  				<Input id="password" label='Password' msg={this.state.password.length < 10 ? 'Min length is 10' : null} onChange={event => this.inputChangeHandler(event, 'password')} type={this.state.passwordVisible ? 'text' : 'password'}></Input>
  	...
  	return (
  		<div >
  			<Button variant="outline-info" onClick={this.submitHandler}
  				...
  				disabled={this.state.password.length < 10}
  			>
  				Request
  			</Button>
  		</div>
