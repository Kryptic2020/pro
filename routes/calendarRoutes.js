...
module.exports = (app) => {
  ...
  //CREATE SPECIALTY
  app.post('/api/specialty/post', requireLogin, async (req, res) => {
    const { specialty } = req.body;
    const sc = await Specialties.find({ name: specialty });
    if (sc.length > 0) {
  ...
  app.get('/api/specialties/get', requireLogin, async (req, res) => {
    const sp = await Specialties.find({});
    res.send(sp);
  });

  //DELETE SPECIALTY
  app.post('/api/specialty/delete', requireLogin, async (req, res) => {
    const { _id } = req.body;
  ...
  //CREATE SERVICEPRICE
  app.post('/api/serviceprice/post', requireLogin, async (req, res) => {
    const { name, price, assignmentID, serviceDetails } = req.body;
    const sc = await ServicePrice.find({ name });
    if (sc.length > 0) {
      res.send('This service name already exist!!!');
    } else {
  ...
  app.get('/api/serviceprice/get', requireLogin, async (req, res) => {
    const sc = await ServicePrice.find({});
    res.send(sc);
  });

  //DELETE SERVICEPRICE
  app.post('/api/serviceprice/delete', requireLogin, async (req, res) => {
    const { _id } = req.body;
  ...
  //DELETE SERVICEPRICE
  app.post('/api/serviceprice/update', requireLogin, async (req, res) => {
    const { _id, price } = req.body;
    await ServicePrice.updateOne({ _id }, { price });
    res.send('Price Updated!!');
  });

  //CREATE STAFF ASSIGNMENTS
  app.post('/api/staffassignments/post', requireLogin, async (req, res) => {
    const { staff, staffID, assignedSpecialty, assignedServices } = req.body;

    const exist = await StaffAssignments.find({ staffID, assignedSpecialty });
    if (exist.length > 0) {
      res.send('Staff already assigned for this specialty!');
    } else {
      ...
      await s.save();
      res.send('Staff Assignments Successfully Created!!!');
    }
  });

  //FETCHING STAFF ASSIGNMENTS
  app.get('/api/staffassignments/get', requireLogin, async (req, res) => {
    const sp = await StaffAssignments.find({});
    res.send(sp);
  });

  //DELETE STAFF ASSIGNMENTS
  app.post('/api/staffassignments/delete', requireLogin, async (req, res) => {
    const { _id } = req.body;
    await StaffAssignments.findByIdAndRemove({ _id });
    res.send('Deleted!!');
  });

  app.get('/api/timeTable/get', requireLogin, async (req, res) => {
    const table = await T.find({ _user: req.user.id });
    res.send(table);
  });

  app.delete('/api/timeTable/:id', requireLogin, async (req, res) => {
    const table = await T.findByIdAndRemove(req.params.id);
    res.send('Deleted!!');
  });

  //CREATE TIME TABLE
  app.post('/api/createTimeTable', requireLogin, async (req, res) => {
  ...
  app.post('/api/calendar/post', requireLogin, async (req, res) => {
    const {
      times,
      toLocaleArray,
      specialty,
    ...
    times.times.forEach(async (T) => {
      toLocaleArray.forEach(async (D) => {
        await NewCalendar.updateOne(
          { date: D, time: T, specialty, staffID },
          { $set: { openView } }
        ).exec();
  ...
  app.post('/api/calendar/delete', requireLogin, async (req, res) => {
    const { times, toLocaleArray, specialty, staffID } = req.body;
    times.times.forEach(async (T) => {
      toLocaleArray.forEach(async (D) => {
        const calendar = await NewCalendar.findOne({
          date: D,
        ...
        if (calendar) {
          await NewCalendar.deleteOne({ _id: calendar._id });
        } else {
        }
      });
    });
  ...
  app.get('/api/calendar/get', requireLogin, async (req, res) => {
    const cal = await NewCalendar.find({});
    res.send(cal);
  });
  //FETCHING CALENDAR BY SPECIALTY
  app.post('/api/calendar/byspecialty', requireLogin, async (req, res) => {
    const { specialty } = req.body;
    const byspecialty = await NewCalendar.find({ specialty });
    res.send(byspecialty);
  });

  //FETCHING CALENDAR BY DATE&SERVICE
  app.post('/api/calendar/bydates', requireLogin, async (req, res) => {
    const { specialty, startDate, endDate, staffID } = req.body;
  ...
  app.post('/api/calendar/from-today', requireLogin, async (req, res) => {
    const { date } = req.body;
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      res.send('Well Done! You can see your Booking at MyBookings');
      let zica = '';
      id.map((x) => {
        zica = x._id;
      });
      ...
      const dateAUS = {
        date: new Date(parseISO(booking.date)).toLocaleDateString('es-ES', {
          year: 'numeric',
          month: 'numeric',
          day: 'numeric',
        }),
      ...
      async function bookingConfirmation() {
        let info = await transporter.sendMail({
          ...
          subject: 'Booking Confirmation  - ( DO NOT REPLY ) ✔', // Subject line
          text: 'Booking Confirmation  - ( DO NOT REPLY )', // plain text body
          html: bookingNotification(user, booking, dateAUS), // html body
        });
      }
  ...
  //POSTING NEW BOOKING
  app.post('/api/booking/admnew', requireLogin, async (req, res) => {
    const {
      date,
      time,
      specialty,
    ...
    if (newBooking) {
      res.send('FAILED :  Date&Time Already Exist!');
    } else {
  ...
  //FETCHING MY BOOKING
  app.get('/api/booking/mybooking', requireLogin, async (req, res) => {
    let data = [];
  ...
  app.post('/api/booking/remainingspots', requireLogin, async (req, res) => {
  ...
  //CANCEL BOOKING
  app.post('/api/booking/cancel', requireLogin, async (req, res) => {
    const { _id, specialty, date, time, service, bookedByName } = req.body;
    ...
    async function bookingCancellation() {
      let info = await transporter.sendMail({
        from: keys.sender, // sender address
        to: email, // list of receivers
        subject: 'Your Booking has been cancelled  - ( DO NOT REPLY )', // Subject line
        text: 'Your Booking has been cancelled  - ( DO NOT REPLY )', // plain text body
        html: bookingCancelation(user, booking, dateAUS), // html body
      });
  ...
  //CREATE WAITING LIST
  app.post('/api/booking/waitinglist', requireLogin, async (req, res) => {
    const {
      date,
      time,
      specialty,
    ...
    async function waitingList() {
      let info = await transporter.sendMail({
        ...
        html:
          '<p> Date: ' +
          parseISO(date).toLocaleDateString('es-ES', {
            year: 'numeric',
            month: 'numeric',
            day: 'numeric',
          }) +
  ...
  app.get('/api/booking/waitinglist/get', requireLogin, async (req, res) => {
    //let data = [];
    const myWaitingList = await WaitingList.find({}).sort('date');
  ...
  //UPDATE COMMENTS
  app.post('/api/calendar/comments/update', requireLogin, async (req, res) => {
  ...
  //DELETE COMMENTS
  app.post('/api/calendar/comments/delete', requireLogin, async (req, res) => {
    const { _id } = req.body;
    await NewCalendar.updateOne(
      {
        _id,
