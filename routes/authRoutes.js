...
module.exports = (app) => {
  ...
  app.post('/api/register', async (req, res) => {
    ...
    const { email, fullName, photo, phone, password, provider } = req.body;
      ...
      if (!existingUser) {
        ...
        async function newRegistered() {
          let info2 = await transporter.sendMail({
            ...
            subject: 'New user has been registered into our app ✔', // Subject line
            text: 'New user has been registered into our app', // plain text body
  ...
  app.post('/api/auth/verifyemail/resend', async (req, res) => {
    const { email } = req.body;
    const user = req.user;
    console.log(req.body);
    const resetPassUser = await User.findOne({ email });
    if (resetPassUser) {
      res.send('Re-sent!!!       Please check your inbox and spam box');
      async function signUp() {
        let info = await transporter.sendMail({
          from: keys.sender, // sender address
          to: email, // list of receivers
  ...
  app.get('/api/emailvalidation/:id', async (req, res) => {
    ...
    const found = await User.updateOne(
      { token },
      { emailVerified: true, token: randomToken(50) },
      function (err, docs) {
        if (err) {
          console.log(err);
  ...
  app.post('/api/newpass', async (req, res) => {
    ...
    const hash = bcrypt.hashSync(password, 10);
    await User.updateOne({ token }, { password: hash, token: randomToken(50) })
  ...
  app.post('/api/theme-update', async (req, res) => {
    ...
    await User.updateOne({ _id: req.user._id }, { theme: color });
  });
  ...
  app.post('/api/auth/daysCalendarView', async (req, res) => {
    const { _id } = req.body;
		const daysCalendarView = await User.findOne({ _id})
    const days = { days: daysCalendarView.daysCalendarView };
		res.send(days);
	});  
};
